...
# --- GLOBAL & GİRİŞ CSS & JS STİLLERİ ---import streamlit as st
import pandas as pd
import os
import base64
from datetime import datetime, timedelta
...
if st.session_state.aktif_sayfa == "Ana Sayfa":
    ...
    tablo_verileri.append({"Sınıf No": 1, "Harç (TL)": h1, "Avukat (TL)": a1, "Sınıf Toplam Harç Ücreti": h1 + a1})

    h2 = giris_h1 + giris_h1
    a2 = ortak_avukat_input
    tablo_verileri.append({"Sınıf No": 2, "Harç (TL)": h2, "Avukat (TL)": a2, "Sınıf Toplam Harç Ücreti": h2 + a2})

    h3 = giris_h1 + giris_h1 + giris_h3_bedel
    a3 = ortak_avukat_input
    tablo_verileri.append({"Sınıf No": 3, "Harç (TL)": h3, "Avukat (TL)": a3, "Sınıf Toplam Harç Ücreti": h3 + a3})
    ...
    for i in range(4, 46):
        ...
        tablo_verileri.append({"Sınıf No": i, "Harç (TL)": hi, "Avukat (TL)": ai, "Sınıf Toplam Harç Ücreti": hi + ai})
    ...
    st.data_editor(
        config_df,
        column_config={
            "Sınıf No": st.column_config.NumberColumn("Sınıf", disabled=True),
