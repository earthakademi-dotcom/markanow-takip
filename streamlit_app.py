import streamlit as stimport pandas as pdimport osimport jsonimport base64from datetime import datetime# --- SAYFA YAPILANDIRMASI ---

st.set_page_config(page_title="Markanow ERP", layout="wide")# --- GLOBAL CSS (SOL MENÜ BORDO, YAZILAR BEYAZ) ---
...
SINIFLAR = [str(i) for i in range(1, 46)] + [f"35/{i}" for i in range(1, 35)]def load_data():

if os.path.exists(DATA_FILE):

return pd.read_csv(DATA_FILE)

return pd.DataFrame(columns=["ID", "Marka Adı", "Ad Soyad", "TC", "Telefon", "Doğum Tarihi", "İl", "Sınıf", "Ödeme", "Satış Tarihi", "Tutar", "Durum", "Danışman", "Fatura No"])# --- GİRİŞ ---

logo_path = "sosyalmedya-2.jpg.jpg"if os.path.exists(logo_path):
...
st.markdown(page_bg_img, unsafe_allow_html=True)else:
...
st.session_state.kullanici = Noneif not st.session_state.kullanici:
...
st.sidebar.write(f"👤 Aktif: **{st.session_state.kullanici}**")if st.sidebar.button("🚪 Güvenli Çıkış", use_container_width=True):
...
menu_options = ["📝 Satış Girişi", "📊 Satışlarım"]if st.session_state.kullanici in ["SELEN AKCAN", "ALİ OSMAN YELBEY", "DENİZ TELLİ GÜRLEYENDAĞ"]:

menu_options.extend(["📥 Excel'den Yükle", "💰 Muhasebe Onayı"])if st.session_state.kullanici in ["ALİ OSMAN YELBEY", "DENİZ TELLİ GÜRLEYENDAĞ"]:
...
df = load_data()# --- MODÜLLER ---if menu == "📝 Satış Girişi":
...
st.success("Satış kaydedildi.")elif menu == "📊 Satışlarım":

st.header(f"📊 {st.session_state.kullanici} - Satışlarım")

st.dataframe(df[df['Danışman'] == st.session_state.kullanici], use_container_width=True)elif menu == "📥 Excel'den Yükle":
...
st.success("Tüm satışlar başarıyla aktarıldı!"); st.rerun()elif menu == "💰 Muhasebe Onayı":
...
df.loc[df['ID'] == row['ID'], 'Durum'] = "Onaylandı"; df.to_csv(DATA_FILE, index=False); st.rerun()elif menu == "📊 Performans Raporu":

st.header("📊 Performans"); st.bar_chart(df[df['Durum'] == "Tamamlandı"].groupby('Danışman')['Tutar'].sum())elif menu == "👥 Personel Yönetimi":
